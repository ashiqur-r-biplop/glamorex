                ...
                <Tabs>
                    <TabList className={'flex gap-5 items-stretch my-5 border-b border-[#0621bb6b]'}>
                        {['All Users', 'customer', 'seller', 'admin'].map((elem, ind) => {
                            return <Tab key={ind} onClick={() => setUserCategory(elem)}
                                className={`py-2 first-letter:uppercase cursor-pointer outline-none ${userCategory === elem
                                    ? "!border-b-2 !border-[#0621bb] !text-[#0621bb]" : 'border-none'
                                    }`}>{elem}</Tab>
                        })}
                    </TabList>
                            ...
                            <div className="overflow-x-auto mt-10">
                                <table className="table">
                                    ...
                                    <tbody>
                                        {!isLoading &&
                                            users
                                                .filter((user) =>  userCategory === 'All Users' ? true : user.userRole === userCategory).map((user, i) => {
                                                    const { _id, name, userRole: role, photo_url, email } = user || {};
                                                    return (
                                                        <tr key={_id}>
                                                            <th>{i + 1}</th>
                                                            <td>
