...
const ProductManagementPage = () => {
    ...
    const search = (data) => {
        axiosSecure
            .get(`/admin/search-products?query=${data.search_text}`)
            .then((res) => {
    ...
    return (
        <div className="p-3">
            <div className="my-8 bg-slate-50 shadow rounded p-5">
                ...
                <Tabs>
                    <TabList className={'flex gap-5 items-stretch my-5 border-b border-[#0621bb6b]'}>
                        {['All Product', 'pending', 'approved', 'rejected'].map((elem, ind) => {
                            return <Tab key={ind} onClick={() => setProductCategory(elem)}
                                className={`py-2 first-letter:uppercase cursor-pointer outline-none ${productCategory === elem ? "!border-b-2 !border-[#0621bb] !text-[#0621bb]" : 'border-none'
                                    }`}>{elem}</Tab>
                        })}
                    </TabList>
                            ...
                            <div className="overflow-x-auto mt-10">
                                <table className="table">
                                    ...
                                    <tbody>
                                        {!isLoading &&
                                            products
                                                .filter((product) => productCategory === 'All Product' ? true : product.product_status === productCategory).map((product, i) => {
                                                    const {
                                                        _id,
                                                        name,
                                                        image,
