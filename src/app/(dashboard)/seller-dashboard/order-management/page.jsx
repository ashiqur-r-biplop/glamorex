            ...
            <div className='my-8 bg-slate-50 shadow rounded p-5'>
                ...
                <div className='relative '>
                    <ul className='flex gap-5 items-stretch my-5 py-2'>
                        <li className='border-b-2 border-[#0621bb] text-[#0621bb] py-2 uppercase'>All Orders</li>
                        ...
                        <tbody>

                            {
                                orders.map((invoice, ind) => {
                                    const { invoiceNo, _id, payment_via, customerName, itemsName, orderDate, status, price } = invoice || {}
                                    ...
                                    return <tr key={ind}>
                                        ...
                                        <td>{orderDate}</td>
                                        <td>${price}</td>
                                        <td ><span className={`${status === "pending" ? "bg-[#fcefcc] text-[#f0ad00]" : status === "completed" ? "bg-[#daebdb] text-[#0a7815]" : status === "retured" ? "bg-[#fce6e8] text-[#e02627]" : "text-[#597eaa] bg-[#a7c3e6]"} px-3 py-1 rounded`}>{status}</span></td>
                                        <td>{payment_via}</td>
                                        <td>
                                        <select defaultValue={status} onChange={(e) => updateOrderStatus(e.target.value, _id)} className='px-4 py-2 border bg-none' name="" id="">
