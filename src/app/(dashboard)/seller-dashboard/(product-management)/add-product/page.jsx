...
const AddProductPage = () => {
    ...
    const { setSelectedSizes, variants, setVariants, handleSizesChange, handleQuantityChange } = useVariant()
    ...
    const onSubmit = (data) => {
        ...
        const updatedVariants = variants.map(variant => ({
            ...variant, sizes: Object.fromEntries(Object.entries(variant.sizes).filter(([size, quantity]) => quantity !== 0))
        }))

        const { name, quantity, category, sub_category, gender, sizes, price, discount, description } = data
        const product = { name, image: imageLink, quantity: totalQuantity || quantity, seller_name: user?.name, seller_email: user?.email, category, sub_category, gender, variants: updatedVariants, price, discount, description };
        console.log(product);
        axiosSecure.post('/add-product', { product }).then(res => {
            setLoading(false);
    ...
    const isGeneralInfoDone = watch('name') && watch('category') && watch('sub_category') && watch('productCoupon') && watch('gender') && ((!isColorsPass && !isSizePass) ? watch('quantity') : true) && (isColorsPass ? variants?.length : true) && (isSizePass ? variants?.length : true)
            ...
            <div className="px-8 py-4">
                ...
                <form onSubmit={handleSubmit(onSubmit)} ref={formRef}>
                    ...
                    {currentPageStatus === "General Information" && (
                                ...
                                <div className="md:flex space-y-6 md:space-y-0 gap-6 lg:gap-14 justify-between">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <select className="my-inp" {...productSubCategoryField}
                                            ...
                                            {
                                                Object.keys(categories?.[watch('category')] || {})?.map((subCategory, ind) => {
                                ...
                                {/* variants */}
                                {
                                    (isColorsPass || isSizePass) && <div className="md:flex space-y-6 md:space-y-0 gap-6 lg:gap-14 justify-between">
                                        ...
                                        {isColorsPass && <div className="flex-1">
                                            <label className="block mb-2 text-sm font-medium text-slate-600 my-subtitle">Variant</label>
                       ...
                       /> */}
                                                        <li className="my-subtitle list-none">{color}</li>
                                                        {/* <label htmlFor={color}>{color}</label> */}
                                                        {
                                                            //    selectedColors.some(sColor => sColor === color) &&
                                                            <>
                                                                <h2 className="text-lg font-semibold !text-orange-500 mx-4">Size:</h2>
                                                                <div className="flex gap-8">
                                                                    {categories[watch('category')]?.[watch('sub_category')]?.sizes?.map((size, ind) => {
                                                                        return <span className="flex items-center gap-2" key={ind}>
                                                                            <input type="checkbox" className="checkbox checkbox-success" id={size + color} value={size} {...register("sizes", { required: true })}
                                                                                onChange={() => { handleSizesChange(size, color) }}
                                                                            />

                                                                            <label htmlFor={size + color}>{size}</label>
                                                                            {/* quantity field */}
                                                                            {
                                                                                variants?.find(variant => variant?.color === color && Object.keys(variant?.sizes).includes(size)) &&
                                                                                <input type="number" placeholder="Quantity..." className="my-inp-quantity"
