...
const productDetailsPage = ({ params }) => {
    ...
    useEffect(() => {
        axios('/products.json').then(
            res => {
                const extractProduct = res.data.find(p => p.product_id === id)
                const similarProducts = res.data.filter(p => p.category === extractProduct.category)
    ...
    if (loading) {
        return <h2 className='my-title h-screen flex items-center justify-center bg-emerald-300'>Loading...</h2>
    }

    const { name, image, description, colors, keyFeatures, product_id, seller_name, seller_email, rating, review, previous_price, discount, price, quantity, overall_sell, publish_date, category, label, status } = product || {}
    ...
    return (
        <div className='bg-slate-100 py-6'>
            ...
            <div className='my-container'>
                <h2 className='flex items-center gap-2 text-lg'> <Link href={'/'}><FaHome></FaHome></Link> / <Link href={`/`}>{category}</Link> / <Link href={`/`}>{name}</Link> </h2>
                    ...
                    <div className='col-span-12 lg:col-span-5 bg-[#ffffff] flex items-center'>
                        <figure className='rounded-lg w-full h-[450px] relative z-10 cursor-zoom-in'>
                            {/* <Image
                                ...
                                fill
                            /> */}
                            <ReactImageZoom width={500} height={500} zoomWidth={300} img={image} />
                        </figure>
                    </div>
